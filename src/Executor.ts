...
export default class Executor implements IExecutor {
  ...
  public start() {
    ...
    this.recordPerformanceInterval = setInterval(
  ...
  public stop() {
    ...
    this.performanceReport.max = this.executeData.performanceData.reduce(
      (max, record) => {
    ...
    this.performanceReport.avg = this.executeData.performanceData.reduce(
      (avg: number, record) => {
